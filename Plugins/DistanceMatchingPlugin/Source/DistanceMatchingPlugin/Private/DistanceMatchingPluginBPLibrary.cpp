...
float UDistanceMatchingPluginBPLibrary::GetCurveTime(const UAnimSequence* AnimationSequence, const FName CurveName,
	const float CurveValue)
{
	if(AnimationSequence == nullptr)
	{
		UE_LOG(LogTemp , Error , TEXT("Invalid Animtion Sequence ptr"));
		return 0.0f;
	}
...
bool UDistanceMatchingPluginBPLibrary::PredictStopLocation(FVector& OutStopLocation, const FVector& CurrentLocation,
                                                           const FVector& Velocity, const FVector& Acceleration, float Friction, float BrakingDeceleration,
                                                           const float TimeStep, const int MaxSimulationIterations)
{
